<script lang="ts">
	...
	function setParam(key: string, value: string) {
		...
		params.delete('edit');
		params.delete('new');
		goto(`/suites?${params}`, { keepFocus: true, noScroll: true, replaceState: true });
	}
...
{#if data.editor}
	<div class="table-area">
		<div class="toolbar">
			...
			<a class="btn btn-ghost" href="/suites"><Icon name="arrow-right" class="flip" /> All suites</a>
		</div>
		<SuiteEditor
...
{:else}
	<div class="table-area">
		<div class="toolbar">
			...
			<div class="toolbar-spacer"></div>
			<a class="btn btn-primary" href="/suites?new=1"><Icon name="plus" /> New suite</a>
		</div>
		...
		<div class="filter-bar">
			...
			{#if filterActive}
				<a class="btn btn-ghost btn-sm" href="/suites"><Icon name="x" /> Reset</a>
			{/if}
		</div>
				...
				<div class="suite-grid">
					{#each data.cards as s (s.id)}
						<div class="suite-card tone-card" data-tone={s.tone}>
							<a class="suite-hd" href="/suites?edit={s.id}" title={s.description || s.name}>
								<div class="sh-top">
							...
							<div class="suite-bd">
								...
								{#if s.lastRun}
									<a class="lastrun" href="/runs/{s.lastRun.runId}" title="Open {s.lastRun.runId}">
							...
							<div class="suite-foot">
								...
								<a class="btn btn-ghost btn-sm" href="/runs?suite={s.id}" aria-label="Run history" title="This suite's run history — archive or clean up its runs">
									<Icon name="play" />{#if s.runCount}<span class="foot-n">{s.runCount}{#if s.archivedRuns}·{s.archivedRuns}★{/if}</span>{/if}
								</a>
								<a class="btn btn-ghost btn-sm" href="/suites?edit={s.id}" aria-label="Edit suite" title="Edit suite"><Icon name="edit" /></a>
				...
				<div class="empty">
					<div class="empty-in">
						...
						{#if filterActive}
							...
							<a class="btn btn-primary" style="margin-top:14px" href="/suites"><Icon name="x" /> Reset filter</a>
						{:else}
							<h3>No suites yet</h3>
							<p>Group cases into a suite to run them together — mix API, e2e, visual and manual under one pass rate.</p>
							<a class="btn btn-primary" style="margin-top:14px" href="/suites?new=1"><Icon name="plus" /> New suite</a>
						{/if}
					</div>
				</div>
			{/if}
