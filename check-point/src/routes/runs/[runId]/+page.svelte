<script lang="ts">
	...
	// Cross-app issue linking is available only when a central IssueDesk is wired
	// up (ISSUEDESK_URL). Otherwise failures export as a prompt instead.
	const issuedeskUrl = $derived(data.issuedeskUrl as string | null);
	const issuedeskConfigured = $derived(data.issuedeskConfigured as boolean);
	...
	async function closeRun() {
		completing = true;
		try {
			const res = await fetch(`/runs/${run.id}?/completeRun`, {
				method: 'POST',
	...
	async function post(action: string, caseId: string, extra: Record<string, string> = {}) {
		...
		const res = await fetch(`/runs/${run.id}?/${action}`, {
			method: 'POST',
			headers: { 'x-sveltekit-action': 'true' },
			body
		});
...
{#snippet rowActions(row: { status: ResultStatus; testCaseId: string; issueId: string | null })}
	{#if isFail(row.status)}
		{#if row.issueId}
			{#if issuedeskUrl}
				<a class="issue-chip" href="{issuedeskUrl}/issues/{row.issueId}">{row.issueId}</a>
			{:else}
				<span class="issue-chip">{row.issueId}</span>
			{/if}
...
<div class="table-area">
	<div class="toolbar">
		...
		{/if}
		<a class="btn btn-ghost" href="/runs">All runs</a>
	</div>
