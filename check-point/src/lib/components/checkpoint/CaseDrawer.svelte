<script lang="ts">
	...
	// The central IssueDesk base URL, or null when this Checkpoint runs
	// standalone — issue ids then render as plain text, not links.
	const issuedeskUrl = $derived(page.data.issuedeskUrl as string | null);
	const issueHref = (id: string) => (issuedeskUrl ? `${issuedeskUrl}/issues/${id}` : null);
		...
		<div class="dr-body">
			{#if c.parentIssueId}
				<div class="xlink-note">
					...
					<div>
						Parent issue
						{#if issueHref(c.parentIssueId)}<a class="parent-chip" href={issueHref(c.parentIssueId)} style="margin:0 3px">{c.parentIssueId}</a>{:else}<span class="parent-chip" style="margin:0 3px">{c.parentIssueId}</span>{/if}
			...
			<div class="dr-meta">
				...
				{#if drawer.filedIssues.length}
					<div class="row">
						...
						<div class="rv">
							{#each drawer.filedIssues as id (id)}
								{#if issueHref(id)}<a class="issue-chip" href={issueHref(id)}>{id}</a>{:else}<span class="issue-chip">{id}</span>{/if}
							{/each}
						</div>
					</div>
				{/if}
