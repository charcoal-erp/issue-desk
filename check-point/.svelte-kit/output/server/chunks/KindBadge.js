import { C as attr, T as escape_html, c as stringify, d as html, r as derived, t as attr_class, w as clsx } from "./server.js";
import { a as TEST_KIND_META } from "./meta.js";
...
function Icon($$renderer, $$props) {
	...
	$$renderer.push(`<svg${attr_class(clsx(cls))} viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor"${attr("stroke-width", icon().sw)} stroke-linecap="round" stroke-linejoin="round">${html(icon().paths)}</svg>`);
}
//#endregion
//#region src/lib/components/checkpoint/KindBadge.svelte
function KindBadge($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { kind, small = false } = $$props;
		const KIND_ICON = {
			unit: "flask",
			api: "code",
			e2e: "monitor",
		...
		$$renderer.push(`<span${attr_class(`kind ${stringify(TEST_KIND_META[kind].cls)}`, void 0, { "sm": small })}>`);
		Icon($$renderer, { name: KIND_ICON[kind] });
		$$renderer.push(`<!----> ${escape_html(TEST_KIND_META[kind].label)}</span>`);
	});
}
//#endregion
export { Icon as n, KindBadge as t };
