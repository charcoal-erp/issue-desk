...
export const load = async ({ params }: Parameters<PageServerLoad>[0]) => {
	...
	const now = new Date();
	const userName = (id: string) => config.users().find((u) => u.id === id)?.name ?? id;
...
export const actions = {
	recordResult: async ({ request, params }: import('./$types').RequestEvent) => {
		await cp.ensureLoaded();
	...
	completeRun: async ({ params }: import('./$types').RequestEvent) => {
		await cp.ensureLoaded();
		try {
	...
	// Fail → bug: file an issue in the central IssueDesk over HTTP, prefilled
	// from the failed result, and link both sides (design §13). Only available
	// when ISSUEDESK_URL is configured.
	createBugFromResult: async ({ request, cookies, params }: import('./$types').RequestEvent) => {
		if (!issuedesk.isConfigured()) return fail(400, { error: 'No IssueDesk is configured.' });
		await cp.ensureLoaded();
		...
		try {
			...
			const payload = { ...resultToIssueInput(c, result, run, runner), testCaseId: c.id, runId: run.id };
			const issue = await issuedesk.createIssue(payload, actor(cookies));
