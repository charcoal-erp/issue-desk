...
IssueDesk is one of two separate apps in this repo — see the [root README](../README.md).
Checkpoint (test management) lives in [`../check-point/`](../check-point/) and is a wholly
independent application; the two share no code.
...
- `ISSUEDESK_INGEST_TOKEN` — when set, `POST /api/issues` requires it as a bearer token, so
  only a trusted Checkpoint (or the simulators) can file issues.
...
| Var | Default | Purpose |
|---|---|---|
| `DATA_DIR` | `../data` | Root of all config / issues / uploads (the repo keeps its dev data at the repo root) |
...
| `ISSUEDESK_INGEST_TOKEN` | _(unset)_ | Optional — when set, `POST /api/issues` requires this bearer token |
...
```
data/                  # DATA_DIR — the issue tracker's data (repo keeps it at the repo root)
