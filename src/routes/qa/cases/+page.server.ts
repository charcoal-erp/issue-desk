...
export const load: PageServerLoad = async ({ url }) => {
	...
	const issues = store.list({}).rows.map((i) => ({ id: i.id, title: i.title, appId: i.appId }));
	const nextCaseIds = Object.fromEntries(store.applications().map((a) => [a.id, cp.nextCaseId(a.id)]));
	...
	if (drawerId) {
		...
		if (c) {
			...
			const parent = c.parentIssueId ? store.get(c.parentIssueId) : undefined;
			drawer = {
				case: c,
				last: last
					? {
