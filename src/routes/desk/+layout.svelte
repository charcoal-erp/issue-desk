<script lang="ts">
	...
	const NAV = [
		{ href: '/desk', label: 'Issues', icon: 'rows' },
		{ href: '/desk/board', label: 'Board', icon: 'board' },
		{ href: '/desk/metrics', label: 'Metrics', icon: 'dashboard' }
	];

	function isActive(href: string): boolean {
		return href === '/desk' ? page.url.pathname === '/desk' : page.url.pathname.startsWith(href);
	}
...
<div class="app">
	<TopBar bind:this={topbar} workspace="desk" users={data.users} currentUserId={data.currentUserId} />
	<div class="body">
		<nav class="nav">
			...
			<a class="nav-item" class:active={isActive('/desk/admin')} href="/desk/admin">
				<Icon name="gear" />
				<span>Config</span>
			</a>
		</nav>
