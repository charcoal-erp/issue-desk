<script lang="ts">
	...
	import type { Issue, IssueFilter, Status } from '$lib/types';
	...
	// Status is what this page *is*, so it survives every navigation and never
	// appears as a chip the visitor could remove.
	const chipFilter = $derived({ ...data.filter, status: undefined });

	function navigate(patch: Partial<IssueFilter>, replace = false) {
		const next: IssueFilter = { ...data.filter, ...patch, status: BACKLOG };
		goto(`/backlog?${filterToParams(next)}`, { keepFocus: true, noScroll: true, replaceState: replace });
	}
	...
	function clearAll() {
		quick = '';
		goto('/backlog', { keepFocus: true, noScroll: true });
	}
