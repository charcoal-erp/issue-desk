 ...
 * in .backups/pre-import-<stamp>/), then reloads both in-memory stores.
 * Checkpoint content on disk is not touched.
 */
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		// Issue store first — Checkpoint denormalises from its applications.
		await store.reload();
