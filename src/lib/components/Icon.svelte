<script lang="ts">
	// Central inline-icon set, paths lifted verbatim from the approved mockup.
	const ICONS: Record<string, { paths: string; sw: number }> = {
		logo: {
			paths:
				'<path d="M12 2l3 3h4v4l3 3-3 3v4h-4l-3 3-3-3H5v-4l-3-3 3-3V5h4z"/><circle cx="12" cy="11" r="2.5" fill="currentColor" stroke="none"/>',
			sw: 2.4
		},
		search: { paths: '<circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/>', sw: 2 },
		chevron: { paths: '<path d="M6 9l6 6 6-6"/>', sw: 2.4 },
		check: { paths: '<path d="M5 12l5 5L20 6"/>', sw: 3 },
		'check-bold': { paths: '<path d="M5 12l5 5L20 6"/>', sw: 3.5 },
		'check-sm': { paths: '<path d="M5 12l5 5L20 6"/>', sw: 2.5 },
		rows: { paths: '<path d="M3 6h18M3 12h18M3 18h18"/>', sw: 2 },
		board: {
			paths:
				'<rect x="3" y="4" width="5" height="16" rx="1.2"/><rect x="10" y="4" width="5" height="11" rx="1.2"/><rect x="17" y="4" width="4" height="14" rx="1.2"/>',
			sw: 2
		},
		dashboard: {
			paths:
		...
		plus: { paths: '<path d="M12 5v14M5 12h14"/>', sw: 2.4 },
		export: {
			paths: '<path d="M12 15V3"/><path d="M7 8l5-5 5 5"/><path d="M5 15v4a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-4"/>',
			sw: 2
		},
		x: { paths: '<path d="M18 6L6 18M6 6l12 12"/>', sw: 2.2 },
		'x-sm': { paths: '<path d="M18 6L6 18M6 6l12 12"/>', sw: 2.4 },
		edit: {
		...
		paperclip: {
			paths:
				'<path d="M21 12.5l-9 9a5 5 0 0 1-7-7l9-9a3.3 3.3 0 0 1 4.7 4.7l-9 9a1.7 1.7 0 0 1-2.4-2.4l8-8"/>',
			sw: 2
		},
		user: { paths: '<circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/>', sw: 2 },
		bug: {
			paths:
				'<path d="M8 2l1.5 2.5M16 2l-1.5 2.5"/><rect x="7" y="6" width="10" height="12" rx="5"/><path d="M12 6v12M4 10h3M17 10h3M4 14h3M17 14h3M5 18l2-1M19 18l-2-1M5 8l2 1M19 8l-2 1"/>',
			sw: 2
		},
		feature: {
			paths:
				'<path d="M9.9 2.6l1.5 4.6h4.9l-4 2.9 1.5 4.6-4-2.9-4 2.9 1.5-4.6-4-2.9h4.9z"/><path d="M12 15v6M9 21h6"/>',
			sw: 2
		},
		upload: {
		...
		download: { paths: '<path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><path d="M5 21h14"/>', sw: 2 },
		refresh: { paths: '<path d="M21 12a9 9 0 1 1-3-6.7"/><path d="M21 3v5h-5"/>', sw: 2 },
		warning: {
		...
		'search-lg': { paths: '<circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/>', sw: 1.6 },
		task: {
	...
	const icon = $derived(ICONS[name] ?? { paths: '', sw: 2 });
</script>
